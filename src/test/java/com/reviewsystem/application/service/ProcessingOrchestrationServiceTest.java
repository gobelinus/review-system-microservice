...
class ProcessingOrchestrationServiceTest {
  ...
  private int maxConcurrentFilesInTest; // To verify @Value in the test itself
  ...
  void setUp() {
    ...
    int testMaxConcurrentFiles = 5;
    orchestrationService =
        new ProcessingOrchestrationService(
            s3Service,
  ...
  @Nested
  @DisplayName("Process New Files Tests")
  class ProcessNewFilesTest {
    @Test
    ...
    void processNewFiles_ShouldHandleS3ServiceException() {
      // Given
      when(s3Service.listUnprocessedFiles())
  ...
  @Nested
  @DisplayName("Cleanup Old Files Tests")
  class CleanupOldProcessedFilesTest {
    @Test
  ...
  @Nested
  @DisplayName("Fetch Metadata Tests")
  class ProcessingDataTest {
    ...
    void getProcessingStatus_ShouldReturnCurrentStatus() {
      ...
      // call below function to invoke fileTrackingService.getTotalProcessedFiles() and other
      // function
      ProcessingOrchestrationService.ProcessingStats stats =
          orchestrationService.getProcessingStats();

      // Then
      assertEquals(ProcessingStatus.IDLE, status);

      assertEquals(stats.getTotalProcessedFiles(), 100L);
      assertEquals(stats.getProcessedFilesToday(), 5L);
