...
class FileTrackingServiceTest {
  ...
  @InjectMocks private FileTrackingService fileTrackingService;
  ...
  class DuplicateFileDetectionTests {
    ...
    void shouldReturnFalseWhenFileExistsButProcessingFailed() {
      ProcessedFile failedFile =
          sampleProcessedFile.builder().processingStatus(ProcessingStatus.FAILED).build();
    ...
    void shouldReturnFalseWhenFileExistsButIsStillProcessing() {
      ProcessedFile processingFile =
          sampleProcessedFile.builder().processingStatus(ProcessingStatus.IN_PROGRESS).build();
    ...
    void shouldReturnTrueWhenFileIsDuplicate() {
      when(processedFileRepository.existsByS3KeyAndEtag(sampleS3Key, sampleEtag)).thenReturn(true);
    ...
    void shouldReturnFalseWhenFileIsNotDuplicate() {
      when(processedFileRepository.existsByS3KeyAndEtag(sampleS3Key, sampleEtag)).thenReturn(false);
  ...
  class FileTrackingRecordCreationTests {
    ...
    void shouldCreateNewTrackingRecordWhenFileDoesNotExist() {
      ...
      when(processedFileRepository.findByS3KeyAndEtag(sampleS3Key, sampleEtag))
          .thenReturn(Optional.empty());
      when(processedFileRepository.save(any(ProcessedFile.class))).thenReturn(sampleProcessedFile);
  ...
  class FileProcessingStatusUpdatesTests {
    ...
    void shouldMarkProcessingAsStarted() {
      ...
      ProcessedFile pendingFile =
          sampleProcessedFile.builder().processingStatus(ProcessingStatus.PENDING).build();

      when(processedFileRepository.findById(fileId)).thenReturn(Optional.of(pendingFile));
      when(processedFileRepository.save(any(ProcessedFile.class))).thenReturn(pendingFile);
      ...
      assertThat(updatedFile.getProcessingStatus()).isEqualTo(ProcessingStatus.IN_PROGRESS);
    ...
    void shouldThrowExceptionWhenMarkingProcessingStartedForNonExistentFile() {
      ...
      when(processedFileRepository.findById(fileId)).thenReturn(Optional.empty());
    ...
    void shouldMarkProcessingAsCompleted() {
      ...
      when(processedFileRepository.findById(fileId)).thenReturn(Optional.of(processingFile));
      when(processedFileRepository.save(any(ProcessedFile.class))).thenReturn(processingFile);
  ...
  class FileProcessingStatusQueryTests {
    ...
    void shouldReturnProcessingStatusWhenFileExists() {
      ...
      Optional<ProcessingStatus> result =
          fileTrackingService.getFileProcessingStatus(sampleS3Key, sampleEtag);
    ...
    void shouldGetFilesByStatus() {
      ...
      List<ProcessedFile> result = fileTrackingService.getFilesByStatus(ProcessingStatus.COMPLETED);
  ...
  class ProcessingStatisticsTests {
    ...
    void shouldGetProcessingStatisticsForprovider() {
      ...
      when(processedFileRepository.countByProviderAndProcessingStatus(
              sampleprovider, ProcessingStatus.PENDING))
          .thenReturn(10L);
      when(processedFileRepository.countByProviderAndProcessingStatus(
              sampleprovider, ProcessingStatus.IN_PROGRESS))
          .thenReturn(5L);
      when(processedFileRepository.countByProviderAndProcessingStatus(
              sampleprovider, ProcessingStatus.COMPLETED))
          .thenReturn(170L);
      when(processedFileRepository.countByProviderAndProcessingStatus(
              sampleprovider, ProcessingStatus.FAILED))
          .thenReturn(15L);

      FileTrackingService.ProcessingStatistics stats =
          fileTrackingService.getProcessingStatistics(sampleprovider);
  ...
  class CleanupOfOldRecordsTests {
    ...
    void shouldCleanUpOldRecordsSuccessfully() {
      ...
      verify(processedFileRepository)
          .deleteByCreatedAtBeforeAndProcessingStatusIn(
    ...
    void shouldRespectCustomCleanupRetentionDays() {
      ...
      ArgumentCaptor<LocalDateTime> dateCaptor = ArgumentCaptor.forClass(LocalDateTime.class);
      verify(processedFileRepository)
          .deleteByCreatedAtBeforeAndProcessingStatusIn(dateCaptor.capture(), anyList());
  ...
  class RecentlyProcessedFilesTests {
    ...
    void shouldGetRecentlyProcessedFilesForprovider() {
      ...
      List<ProcessedFile> result =
          fileTrackingService.getRecentlyProcessedFiles(sampleprovider, hours);

      assertThat(result).isEqualTo(expectedFiles);

      ArgumentCaptor<LocalDateTime> dateCaptor = ArgumentCaptor.forClass(LocalDateTime.class);
      verify(processedFileRepository)
          .findRecentlyProcessedFiles(
      ...
      assertThat(capturedDate)
          .isBetween(expectedSince.minusMinutes(1), expectedSince.plusMinutes(1));
    }
    ...
    void shouldHandleEmptyRecentlyProcessedFiles() {
      ...
      List<ProcessedFile> result =
          fileTrackingService.getRecentlyProcessedFiles(sampleprovider, 12);
  ...
  class ProcessingStatisticsInnerClassTests {
    ...
    void shouldCalculateSuccessRateCorrectly() {
      FileTrackingService.ProcessingStatistics stats =
          FileTrackingService.ProcessingStatistics.builder()
              .totalFiles(100L)
              .completedFiles(85L)
              .failedFiles(15L)
              .build();
  ...
  class ErrorHandlingTests {
    ...
    void shouldHandleRepositoryExceptionsDuringStatusUpdates() {
      ...
      when(processedFileRepository.findById(fileId)).thenReturn(Optional.of(sampleProcessedFile));
      when(processedFileRepository.save(any(ProcessedFile.class)))
          .thenThrow(new RuntimeException("Database connection failed"));
