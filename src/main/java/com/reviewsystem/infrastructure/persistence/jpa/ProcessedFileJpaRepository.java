 ...
 */
@Repository
public interface ProcessedFileJpaRepository extends JpaRepository<ProcessedFile, Long>, ProcessedFileRepository {
    ...
    @Override
    @Query("SELECT pf FROM ProcessedFile pf WHERE pf.createdAt < :cutoffDate AND pf.processingStatus IN :statuses ORDER BY pf.createdAt ASC")
    ...
    @Override
    @Query("SELECT pf FROM ProcessedFile pf WHERE pf.processingStatus = 'PROCESSING' AND pf.processingStartedAt < :cutoffDateTime")
    List<ProcessedFile> findStuckProcessingFiles(@Param("cutoffDateTime") LocalDateTime cutoffDateTime);

    @Override
    @Query("SELECT pf FROM ProcessedFile pf WHERE pf.provider = :provider AND pf.createdAt >= :since AND pf.processingStatus = :status ORDER BY pf.createdAt DESC")
    List<ProcessedFile> findRecentlyProcessedFiles(@Param("provider") String provider,
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM ProcessedFile pf WHERE pf.createdAt < :cutoffDate AND pf.processingStatus IN :statuses")
    int deleteByCreatedAtBeforeAndProcessingStatusIn(@Param("cutoffDate") LocalDateTime cutoffDate,
                                                     @Param("statuses") List<ProcessingStatus> statuses);
}