...
public class HashUtil {
  ...
  /**
   * Generate a simple hash code for the input string Useful for quick hash operations where
  ...
  /**
   * Generate a content hash suitable for duplicate detection Uses SHA-256 but returns a shorter
   * hash for efficiency
   *
   * @param input the string to hash
  ...
  /**
   * Generate hash from multiple input strings Concatenates all inputs with a delimiter before
   * hashing
   *
  ...
  /**
   * Generate a hash suitable for review content deduplication Normalizes the input by trimming,
   ...
   */
  public static String generateReviewContentHash(
      String reviewContent, Integer hotelId, Double rating) {
